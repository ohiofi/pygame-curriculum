...
A fox and a cluster of grapes (a "bunch" of grapes) appear on the screen. Use the arrow keys to move the fox to eat the grapes. When the fox touches the grapes, you receive points and the grapes jump to a random, new location. The game ends after several seconds, the background color changes and the text "Game Over" is displayed. Use a boolean variable called gameOver. If not gameOver then the arrow keys work. Finally, add a slow dog that chases you. If the `dog.colliderect(fox)` then set gameOver to be true.
